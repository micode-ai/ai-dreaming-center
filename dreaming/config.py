...
class AppSettings(BaseSettings):
    ...
    article_locales: str = ""
    # Run by the writing session in the article root, and the publish gate
    # refuses a red result. It has to regenerate EVERYTHING the project's
    # deploy ships, not just the part nearest the article: ai-budget-assistant
    # commits its built site and its deploy assembles committed output, so
    # configuring only `build_blog.py` published article 22 with an apex
    # sitemap.xml that never listed it — the file was regenerated by a second
    # generator (`build_landing.py`) that nothing ran. Chain them with `&&` and
    # name every generated tree in `article_publish_extra_paths`, or the gate
    # will pass on a half-built site.
