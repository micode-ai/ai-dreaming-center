...
async def create_issue(
    *,
    ...
    if body:
        ...
        cmd += ["--body", "(no body — created by AI Dreaming Center)"]
    ...
    if proc.returncode != 0:
        ...
        # gh's most common failures: not logged in, label doesn't exist, repo wrong
        if "not logged into" in msg.lower():
            raise GitHubIssueError("gh CLI не залогинен — выполни `gh auth login`")
        if "could not find label" in msg.lower():
            raise GitHubIssueError(f"Label не существует в репо {repo}: {msg}")
