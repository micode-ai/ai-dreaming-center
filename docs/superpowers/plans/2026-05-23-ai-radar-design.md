...
## Открытые вопросы (нужно подтвердить до старта R2)

1. **Host-проект.** Фича требует одного из импортированных проектов в роли «носителя» команды `/ai-radar-scan`. Создавать отдельный «meta-проект» вроде `ai-radar-host` или переиспользовать существующий (e.g. `ai-dreaming-center` сам по себе, если он импортирован как проект)? Предложение: переиспользовать, добавив проверку «команда существует» при включении флага.
2. **WebFetch квоты.** Сканер пойдёт через Claude CLI с инструментами `WebSearch`/`WebFetch` — упрёмся ли в rate-limits при ~30 источниках раз в неделю? Если да — снизить дефолт до раз в 2 недели, либо разбить на батчи в `_PER_PROJECT_JOBS`-стиле (по 5 источников / запуск).
3. **Многоязычность саммари.** Генерировать оба (`summary_ru`, `summary_en`) сразу или по запросу? Сразу — простой UI, но в 2× токенов на скан. Предложение: оба сразу для wave R2, по запросу — оптимизация на потом.
4. **Где живёт `sources.yaml`** — в `data/ai-radar/` (gitignored, как и остальное `data/`) или в репозитории host-проекта (`{working_dir}/.claude/ai-radar/sources.yaml`)? Первое — проще для ADC, второе — каноничнее по правилу из `CLAUDE.md`. Предлагаю **первое** + опциональный override-путь в конфиге.
