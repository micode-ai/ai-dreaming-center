...
| `check_css_tokens.py` | Дизайн-система: нет цвета в шаблонах, нет литералов в `components.css`, нет неопределённых классов. |
| `smoke_templates_render.py` | Компилирует все шаблоны, затем обходит все GET-роуты без параметров. |
| `smoke_help_sections.py` | Справка и сайдбар перечисляют одни и те же разделы, у каждого есть текст и живой маршрут. |
...
Введена волной D1/D2 (см. [`waves.md`](waves.md)). До неё цвет жил в 464 Tailwind-утилитах светлой темы и 491 инлайн-`style=` по 51 шаблону, а поверх лежал блок `!important`-переопределений, который перекрашивал их в тёмное. Сейчас цвета в шаблонах нет вообще.

### Три файла, в этом порядке

`base.html` подключает их именно так, и порядок значимый:

| Файл | Отвечает за | Что в нём НЕ бывает |
|---|---|---|
| `static/tokens.css` | Единственный источник визуальной правды: `:root` с ~90 custom properties — поверхности, границы, текст, бренд, статусы, тени, спейсинг, радиусы, типографика, плотность таблиц. | Селекторов, кроме `:root`. |
| `static/app.css` | База плюс каркас приложения: `body`, ресеты, фокус-кольца, скроллбары, дефолты для полей ввода и рама `.app-sidebar` / `.app-content`. | Переиспользуемых компонентов контента — им место в `components.css`. |
| `static/components.css` | Семантические классы: `.card`, `.data-table`, `.stat-bar`, `.badge-*`, `.btn-*`, `.field__*`, `.row-actions` и т.д. | **Литералов цвета.** Только `var(--…)`. |

Плюс два узкоспециализированных: `table_tools.css` (сортировка/фильтры таблиц) и `orchestration_swimlane.css`.

### Правило

**Цвет в шаблоне — это баг.** Нужен новый оттенок — заводи токен в `tokens.css`; нужен новый визуальный элемент — заводи класс в `components.css` и ссылайся на токены. Tailwind-утилиты остаются для раскладки (`flex`, `gap-3`, `grid-cols-2`), не для оформления.

Почему так жёстко: Tailwind подключён через Play CDN, который печатает свои правила в `@layer`. Безслойный авторский CSS всегда выигрывает у слойного независимо от специфичности, так что «поправлю точечно утилитой в шаблоне» работает через раз и отлаживается мучительно.
...
Четыре проверки, все обязаны быть в нуле:

1. **Литералы цвета в `components.css`** — hex и любые `rgb()/rgba()/hsl()`, кроме чёрного (чёрный разрешён для теней и оверлеев).
2. **Tailwind-утилиты с цветом из палитры** — `bg-white`, `text-gray-700`, `border-slate-200`. Ловятся все оттенки, не только светлые: тёмному `bg-slate-900` в шаблоне тоже не место. Утилиты размера и раскладки (`text-xs`, `border`) не матчатся — регэксп требует имя цвета.
3. **Статический инлайн-`style=` в шаблонах.** Атрибут с `{{ … }}` внутри — это вычисляемое значение (ширина прогресс-бара), он разрешён. Атрибут, собранный только из `{% %}`, — не разрешён: условный выбор из двух статических цветов принадлежит классу-модификатору.
4. **Класс упомянут в шаблоне, но нигде не определён** — ловит опечатки вроде `text-warnn`. Знает про `<style>`-блоки в шаблонах и про классы, которые навешивает JS.

### Как добавить визуальный элемент

1. Посмотри, нет ли уже подходящего класса в `components.css` — за волну поймали четыре дубля (`.dim` против `.muted`, `.progress-*` против `.meter-*`, два одинаковых модальных селектора, три байт-в-байт совпадающих токена).
2. Не хватает оттенка/размера — добавь токен в `tokens.css`, в свою группу.
3. Добавь класс в `components.css`, только через `var(--…)`.
4. `python scripts/check_css_tokens.py` и `python scripts/smoke_templates_render.py`.

Про контраст: если ставишь цвет текста поверх полупрозрачной подложки, считай контраст против **непрозрачного** предка, а не против токена подложки. Волна нашла около двадцати цветов, которые в исходнике выглядели нормально, а в браузере давали от 2.4:1 до 4:1.

### Даты в таблицах

Ячейка с временем пишется так — сервер рендерит запасной вариант, `static/timeago.js` подменяет его на человеческий:

```jinja
<time data-ts="{{ s.started_at }}">{{ s.started_at[5:16] | replace("T", " ") }}</time>
```

Скрипт даёт относительное время до недели («5 минут назад»), дальше — короткую абсолютную дату, перерисовывает раз в минуту и уважает локаль. Если `data-ts` не парсится, серверный текст остаётся как есть.

### Кэш статики

`dreaming/main.py` кладёт в Jinja-глобалы `asset_v` — максимальный mtime по локальным ассетам, — и `base.html` цепляет его как `?v={{ asset_v }}`. Без этого правки CSS не видны до жёсткого рефреша: браузер держит `/static/*.css` намертво, и перезапуск сервера тут не помогает. Добавляешь новый локальный css/js в `base.html` — впиши имя файла в список внутри `_asset_version()`.
...
- Дизайн-система (спека) — [`superpowers/specs/2026-08-16-design-system-foundation-design.md`](../superpowers/specs/2026-08-16-design-system-foundation-design.md).
