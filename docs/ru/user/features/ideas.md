...
Под заголовками таблицы — строка фильтров:

| Колонка | Тип фильтра |
|---|---|
| id | поиск по подстроке |
| название | поиск по подстроке |
| статус | селект (все / idea / exploring / approved / building / shipped / dropped) |
| приоритет | поиск по подстроке |
| ссылки | селект (все / есть GH / есть run / есть Jira / без ссылок) |
| | кнопка «сбросить» |

Состояние **сохраняется в `localStorage`** под ключом `ideas.filters.{slug}` — переживает перезагрузку и переключение между проектами. При активном фильтре рядом со счётчиком появляется «показано N из M»; если ничего не подошло — «Под фильтр ничего не подходит».

Серверный `?status=...` query param сохранён для совместимости со старыми закладками: применяется до клиентских фильтров, итог — пересечение.

## Массовый запуск (bulk)

Слева у каждой строки — чекбокс. Чекбокс в заголовке — выбрать все **видимые** строки (отфильтрованные не трогаются). Кнопка **«Запустить (N)»** под счётчиком отправляет выбранное в очередь Orchestrator'а — там оно идёт последовательно. Подробнее см. [orchestration.md → Bulk queue](orchestration.md#bulk-queue--массовый-запуск).
