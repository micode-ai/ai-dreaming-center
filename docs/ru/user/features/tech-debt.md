...
Под заголовками таблицы — строка фильтров на все 10 колонок:

| Колонка | Тип фильтра |
|---|---|
| id, название, модуль, сложность, автоном., уверен. | поиск по подстроке |
| status | селект (все / open / in-progress / blocked / closed / dropped) |
| priority | селект (все / critical / high / medium / low) |
| создан | поиск по дате (`YYYY-MM-DD`, подстрока — можно вводить `2026-05` для месяца) |
| refs | селект (все / есть GH / есть run / есть Jira / без ссылок) |
| (actions) | кнопка «сбросить» |

Состояние сохраняется в `localStorage` под ключом `findings.filters.{slug}`. Совместимо с сортировкой (клик по заголовку колонки) и с серверным `?status=...` / `?module=...` query param'ом.

## Массовый запуск (bulk)

Слева у каждой строки — чекбокс. Чекбокс в заголовке — выбрать все **видимые** строки. Кнопка **«Запустить (N)»** под счётчиком отправляет выбранное в очередь Orchestrator'а — там оно идёт последовательно (Orchestrator принимает один run за раз). Подробнее см. [orchestration.md → Bulk queue](orchestration.md#bulk-queue--массовый-запуск).
