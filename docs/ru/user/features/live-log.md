...
После `[stream ended]` карточка показывает статус **«Поток завершён»** ~3 секунды (чтобы успеть прочитать финальные строки), затем плавно скрывается (fade-out 500 мс) и удаляется из DOM. Когда уходит последняя карточка — на её месте появляется заглушка «Ничего не запущено». Перезагрузка не нужна.

Параллельно сервер при GET `/live` фильтрует «зомби»-сессии: если у процесса `returncode is not None` (claude.exe умер, но `_cleanup` ещё не доехал) — карточка не рендерится, а сама запись будет вычищена в фоне. Это спасает от вечно «пульсирующих» карточек без входящих данных.
