...
Канала вопросов у сборщика в `make-creative.md` нет — ни `POST
/api/questions/create`, ни поллинг в тексте команды не упоминаются; писать
факт, который нельзя подтвердить, `make-creative.md` прямо запрещает
("Never invent a number, a customer, a testimonial..."), но альтернативы
кроме честного отчёта о провале команда не предлагает.
